window.onload = function () {
    ...
    //左侧导航部分
    shopShow.addEventListener("mouseover", function (e) {
        var e = e || window.event;
        var target = e.target || e.srcElement;

        while (target !== shopShow) {
            if (target.classList.contains("shopClass_item")) {
                var thisIndex;
                for (var i = 0; i < shopItem.length; i++) {
                    if (shopItem[i] === target) {
                        thisIndex = i;
                    }
                }

                target.classList.add("shopClass_active");
                shopList[thisIndex].style.display = "block";
            }
    ...
    shopShow.addEventListener("mouseout", function (e) {
        var e = e || window.event;
        var target = e.target || e.srcElement;
        ...
        while (target !== shopShow) {
            if (target.classList.contains("shopClass_item")) {
                ...
                target.classList.remove("shopClass_active");
                shopList[thisIndex].style.display = "none";
            }
            target=target.parentNode;
        }
    }, false)
